...
class LLaDABlock(nn.Module):
    ...
    def _scaled_dot_product_attention(
        ...
        local_window_size: Optional[int] = None, # Can also be a tuple (window_size, keep_first_k)
    ) -> torch.Tensor:
        """
        ...
        if local_window_size is not None:
            ...
            local_mask = torch.triu(local_mask, diagonal=-w_size)
            local_mask = torch.tril(local_mask, diagonal=w_size)
            ...
            if attn_mask is None:
                ...
                attn_mask = attn_mask + local_bias
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            att, cache = self._activation_checkpoint_fn(  # type: ignore
                self.attention, q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache, local_window_size=local_window_size
            )
        else:
            att, cache = self.attention(q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache, local_window_size=local_window_size)
        ...
        for block_idx, block in enumerate(self):
            ...
            if (
                ...
                x, cache = self._activation_checkpoint_fn(  # type: ignore
                    block, x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache, local_window_size=local_window_size
                )
            else:
                # shape: (batch_size, seq_len, d_model)
                x, cache = block(x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache, local_window_size=local_window_size)
    ...
    def forward(
        ...
        if self.config.block_group_size == 1:
            ...
            for group_idx, block_group in enumerate(self.transformer.block_groups):
                ...
                x, cache = block_group(
                    x, attention_bias=attention_bias, layers_past=layers_past, use_cache=use_cache, local_window_size=local_window_size
                )
                if attn_key_values is not None:
                    assert cache is not None
