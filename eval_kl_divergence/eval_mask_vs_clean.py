...
def evaluate_mask_vs_clean(model, valid_samples, w_config, mask_ratio, device, pos_id=0):
    mask_jsds = []
    clean_jsds = []
    mask_matches = []
    clean_matches = []
    ...
    with torch.no_grad():
        for seq_idx, input_ids in enumerate(tqdm(valid_samples, desc=f"GPU {pos_id} [Win: {str(w_config)}]", position=pos_id, leave=True)):
            ...
            jsd_all = compute_jsd_tensors(global_probs, local_probs) 
            global_argmax = global_probs.argmax(dim=-1)
            local_argmax = local_probs.argmax(dim=-1)
            matches_all = (global_argmax == local_argmax)

            for idx in possible_indices:
                jsd = jsd_all[idx].item()
                match = int(matches_all[idx].item())
...
def plot_aggregated_results(condition, mask_ratio):
        ...
        if d.endswith(f"_{mask_ratio}") and os.path.isdir(os.path.join(target_root, d)):
            ...
            if os.path.exists(json_path):
                with open(json_path, "r") as root:
                    ...
                    key_jsds = "mask_jsds" if condition == 'MASK' else "clean_jsds"
                    key_matches = "mask_matches" if condition == 'MASK' else "clean_matches"
                    
                    jsds = np.array(data[key_jsds])
                    matches = np.array(data[key_matches])
                    acc = np.mean(matches) * 100 if len(matches) > 0 else 0
                    
                    all_data.append((sort_val, label, jsds, acc))
    ...
    color_map = cm.get_cmap('viridis', len(all_data)+1)
    
    for idx, (sort_val, label, jsds, acc) in enumerate(all_data):
        sorted_jsds = np.sort(jsds)
        p = 1.0 * np.arange(len(sorted_jsds)) / max(len(sorted_jsds) - 1, 1)
        ax1.plot(sorted_jsds, p, label=f'Window={label}', linewidth=2.5, color=color_map(idx))
        
    ax1.set_xlim(0, 0.1)
    ax1.set_ylim(0, 1.0)
    ax1.axvline(x=0.01, color='gray', linestyle='--', label='JSD=0.01 Boundary')
    ax1.set_title(f"JS Divergence CDF: {condition} Tokens (Local vs Global)", fontsize=14, fontweight='bold')
    ...
    bar_color = 'mediumseagreen' if condition == 'MASK' else 'salmon'
    bars = ax2.bar(labels, accs, color=bar_color, edgecolor='black', width=0.4)
    ...
    for bar in bars:
        yval = bar.get_height()
        ax2.text(bar.get_x() + bar.get_width()/2.0, yval + 1.5, f"{yval:.2f}%", ha='center', va='bottom', fontweight='bold', fontsize=12)

    plt.tight_layout(pad=3.0)
    out_path = f"{target_root}/aggregated_{condition.lower()}_ratio{mask_ratio}_2x1.png"
...
def main():
    ...
    if args.plot_only:
        print("\nPlot Only mode activated. Aggregating all MASK AND CLEAN results...")
    ...
    evaluate_mask_vs_clean(model, valid_samples, w_config, mask_ratio=args.mask_ratio, device=args.device, pos_id=args.pos_id)
